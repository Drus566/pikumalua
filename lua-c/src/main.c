...
#include "../lib/lua-5.3/src/lua.h"
#include "../lib/lua-5.3/src/lualib.h"
#include "../lib/lua-5.3/src/lauxlib.h"

// Исполняет скрипт в виртуальной машине (без перекомпиляции можно менять скриптовый код и он будет исполняться)
...
void lua_example_getvar(void) {
    ...
    lua_getglobal(L, "some_var"); // getting back a global variable from Lua
...
void lua_example_call_lua_function(void) {
    ...
    if (lua_isfunction(L, -1)) {
        ...
        lua_pushnumber(L, 4); // 2st function argument, b
...
int native_pythagoras(lua_State* L) {
    ...
    return 1; // return how many values of the function is returning as result to the stack
}
